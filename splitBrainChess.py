...
# This class contains the inner workings of splitBrainChess. If you want to change its settings or start it then
# Please go to splitBrainLauncher.py
# That file also lets you change what engines SplitBrainChess uses.
class SplitBrainChess:
    ...
    # Margin in centipawns of which the left's eval must be better than the right.
    score_margin = None
    ...
    def __init__(self, engineLocation, engineNames, margin):
        ...
        # self.logger = logging.getLogger("splitBrainChess")  

        self.engineFolder = engineLocation
        self.engineFileNames = engineNames
        self.score_margin = margin / 100 # in centipawns, default: 0.5

    # This starts SplitBrainChess.
    def start(self):
        logger.info('Starting SplitBrainChess')
        print_l("SplitBrainChess 1.1 by P. Feldtmann based on CombiChess by T. Friederich")
        ...
        for i in range(0, len(self._engines)):
            try:
                ...
                if i == 0:
                    print_l("info string started engine 0 (" + engineName + ") as left brain")
                else:
                    print_l("info string started engine 1 (" + engineName + ") as right brain")
                
            except Exception as e:
                sys.stderr.write(str(e))
                sys.stderr.write("\nSplitBrainChess Error: could not load the engine at file path: " + self.engineFolder + "/" + self.engineFileNames[i])
                sys.stderr.write(
                    "\n\nDid you change the script to include the engines you want to use with SplitBrainChess?\n")
                sys.stderr.write("To do this, call SplitBrainLauncher.py with argument -e or --enginePath.\n")
                sys.exit()
    ...
    def _mainloop(self):
        ...
        while not exitFlag:
            ...
            if userCommand == "uci":
                print_f("id name SplitBrainChess")
                ...
                for engine in self._engines:
                    engine.terminate()
                print("Bye.")
                logger.info('Exiting SplitBrainChess')
                exitFlag = True
...
#         if score.is_mate():
#             if index == right:
#                 print_and_flush("info string right brain detected mate, stop")
#                 self.listenedTo[right] += 1
#                 bestMove = self._moves[right]
#             else:
#                 print_and_flush("info string left brain detected mate, stop")
#                 self.listenedTo[left] += 1
#                 bestMove = self._moves[left]
#             for info in self._results[index]: 
#                 info.stop()
            
        # if all engines are done, and they agree on a move, do that move
        if self._moves[right] is not None and self._moves[right] == self._moves[left]:
            print_l("info string right and left agree, listening to right")
            self.listenedTo[right] += 1
            self.agreed += 1
            bestMove = self._moves[right]
            diff = self._scores[left] - self._scores[right]
            if diff > 0:
                print_l(self._info[left])
            else:
                print_l(self._info[right])

        # if left is much better than right, do left's move
        elif self._moves[right] is not None and self._moves[left] is not None:
            diff = self._scores[left] - self._scores[right]
            if diff >= self.score_margin:
                print_l("info string listening to left brain; which is stronger by {:2.2f}".format(diff))
                self.listenedTo[left] += 1
                bestMove = self._moves[left]
                print_l(self._info[left])
            elif diff > 0:
                print_l("info string listening to right brain; left is stronger, but not enough, only {:2.2f}".format(diff))
                self.listenedTo[right] += 1
                bestMove = self._moves[right]
                print_l(self._info[right])
            else:
                print_l("info string listening to right brain; left is not stronger")
                self.listenedTo[right] += 1
                bestMove = self._moves[right]
                print_l(self._info[right])

        # all engines are done and they dont agree. Listen to right brain
        elif None not in self._moves:
            print_l("info string listening to right brain, brains dont agree")
            self.listenedTo[right] += 1
            bestMove = self._moves[right]
            print_l(self._info[right])
    ...
    # prints stats on how often was listened to right and how often to left
    def _printStats(self):
        winright, drawright, lossright = get_win_draw_loss_percentages(self._scores_white[1])
        print_l("info string right brain best move: " + str(self._moves[1]) + " score: " + str(self._scores_white[1])
                       + " white {:2.1f}% win, {:2.1f}% draw, {:2.1f}% loss".format(winright, drawright, lossright))
        winleft, drawleft, lossleft = get_win_draw_loss_percentages(self._scores_white[0])
        print_l("info string left brain best move: " + str(self._moves[0]) + " score: " + str(self._scores_white[0])
                      + " white {:2.1f}% win, {:2.1f}% draw, {:2.1f}% loss".format(winleft, drawleft, lossleft))
        print_l("info string listen stats [left brain, right brain] " + str(self.listenedTo))
        totalSum = self.listenedTo[0] + self.listenedTo[1] 
        rightSum = self.listenedTo[1] 
        rightPercent = (float(rightSum) / float(totalSum)) * 100.0
        print_l("info string listen stats right brain {:2.1f} %".format(rightPercent))
        agreedPercent = (float(self.agreed) / float(totalSum)) * 100.0
        print_l("info string right brain and left brain agreed so far " + str(self.agreed) + " times, {:2.1f} % ".format(agreedPercent))
